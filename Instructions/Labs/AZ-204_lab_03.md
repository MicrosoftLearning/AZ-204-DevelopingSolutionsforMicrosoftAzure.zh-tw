...
1. 在 [**建立記憶體帳戶**] 刀鋒視窗的 [基本]** 索引卷標上**，執行下列動作，然後選取 **[下一步：進階**]：
...
1. 在 [ **進階** ] 索引標籤上，確定 **已核取 [允許在個別容器** 上啟用匿名存取]。 檢查設定是否未啟用，然後選取 [ **檢閱**]。
...
1. 在 [**存取金鑰**] 刀鋒視窗上，針對其中一個 **[金鑰**] 文字框選取 **[顯示**]。 無論您使用哪種專案，它們都是可互換的。

1. 檢閱任一個金鑰，然後將其中一**個 [金鑰**] 方塊的值複製到剪貼簿，然後將它貼到 記事本。
...
1. 在終端機中，執行下列命令以匯入 Azure.儲存體 12.18.0 **版。NuGet 中的** Blob：
