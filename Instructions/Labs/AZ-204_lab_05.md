...
1. 切換回到 [從 Visual Studio Code]**** 視窗，然後從 [檔案總管]**** 窗格開啟該 **Dockerfile** 檔案。
...
1. 在 Azure 入口網站中，選取 [Cloud Shell]**** 圖示 ![Cloud Shell 圖示](./media/az204_lab_CloudShell.png)，開啟 Cloud Shell。 如果 Cloud Shell 預設為 PowerShell 工作階段，請選取 [Cloud Shell]**** 選單中的 [切換至 Bash]****，然後選取 [確認]****。
    ...
    如果這是您第一次啟動 **Cloud Shell**，您將會看到一系列用來建立資源的提示。 執行下表中顯示的動作：

    | 提示 | 動作 |
    |--|--|
    | 歡迎使用 Azure Cloud Shell | 選取 [Bash]****。 |
    | 快速入門 | 選取 [掛接儲存體帳戶]****，然後從下拉式清單中選取您的**儲存體帳戶訂閱**，最後選取 [套用]****。 |
    | 掛接儲存體帳戶 | 選取 [我們將為您建立儲存體帳戶]****，然後選取 [下一步]****。 |
...
1. 在 [存放庫] 刀鋒視窗的 [設定]**** 區段中，選取 [存取金鑰]****。

1. 在 [存取金鑰]**** 刀鋒視窗中，選取 [管理使用者]**** 核取方塊。

1. 在 [存放庫] 刀鋒視窗的 [服務]**** 區段中，選取 [存放庫]****。 
