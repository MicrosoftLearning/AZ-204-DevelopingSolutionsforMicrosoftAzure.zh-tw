...
## <a name="ensure-that-your-local-lab-environment-has-its-time-set-properly-no-delay-according-to-your-timezone"></a>請確保已正確設定本機實驗室環境的時間 (根據您的時區，沒有延遲)

> **注意：** 若您注意到本機實驗室環境的時間 (實驗室 VM 或本機電腦) 有延遲，請使用下列 PowerShell 命令增加 15 分鐘，藉此調整其當地時間。 您必須以系統管理員身分執行 powershell。
...
1.  在 [Azure Active Directory] **** 刀鋒視窗上，選取 [管理] 區段中的 [應用程式註冊]。
...
### <a name="exercise-2-create-a-single-tenant-aspnet-web-app"></a>練習 2：建立單一租用戶 ASP.NET Web 應用程式

#### <a name="task-1-create-an-aspnet-web-app-project"></a>工作 1：建立 ASP.NET Web 應用程式專案
...
### <a name="exercise-3-clean-up-your-lab-environment"></a>練習 3：清理實驗室環境
...
1.  在 Azure 入口網站中，瀏覽至 Azure 訂閱的相關聯 Azure AD 租用戶刀鋒視窗。
