...
1. 在 [**建立 Azure Cosmos DB 帳戶**] 刀鋒視窗上，選取 **[適用於 NoSQL** 的 Azure Cosmos DB] 方塊中的 **[建立**]。

1. 在 [**建立 Azure Cosmos DB 帳戶 - 適用於 NoSQL** 的 Azure Cosmos DB] 頁面上的 [基本]** **索引卷標上，執行下列動作，然後選取 [**檢閱 + 建立**]：
...
1. 在 [**建立 Azure Cosmos DB 帳戶 - 適用於 NoSQL** 的 Azure Cosmos DB] 頁面的 **[檢閱 + 建立**] 索引標籤上，檢閱您在上一個步驟中選取的選項。
...
1. 在 [金鑰 **] 窗格中的 **[** 讀寫金鑰**] 索引標籤上，將 [URI **]、**[主要金鑰**] 和 **[主要連接字串**] 文字框的值**記錄到 記事本。 您會在稍後的實驗中用到這些值。

   > **注意**：您必須使用主鍵**和**主要 連線 字串**的**眼睛圖示，才能將它們複製到剪貼簿。 
...
1. 回到 [ **容器]** 區段，流覽至新建立 **的映像** 容器。

1. 在 [容器] 窗格上尋找 [設定] 區段，然後選取 [屬性] 連結。

1. 在 [**屬性]** 窗格中，記下URL文字框中的值**** 並記錄到 記事本。 您會在稍後的實驗中用到此值。

1. 按兩下 **[下一步共用存取令牌** ]，然後按兩下 **[產生 SAS 令牌和 URL**]，以使用預設設定產生存取令牌。 將 Blob SAS 令牌**的值**記錄到 記事本，稍後您將需要此專案。

   > **注意**：您現在應該在 記事本 中有 5 個值。 Cosmos DB 帳戶的 URI、Pimary 金鑰**和主要 連線 ion 字串。 ********** 然後是 Blob 容器的 **URL** 和 **SAS 令牌** 。
...
1. 在**第 14 行的 Program.cs** 檔案中，將空字串取代為您**稍早在此實驗室中記錄的 **Cosmos DB 帳戶** URI** 屬性，以設定 EndpointUrl** 的值**。 請務必將該值括在雙引號中。 這是**您在 記事本 中記錄的第一個**值。

1. 在第 15 行上，將空白字串取代為您**稍早在此實驗室中記錄的 **Cosmos DB 帳戶** PRIMARY KEY** 屬性，以設定 AuthorizationKey** 的值**。 請務必將該值括在雙引號中。 這是**您在 記事本 中記錄的第二個**值。
...
1. 更新 **AdventureWorksCosmosCoNtext** 屬性的值，將其設為您稍早在此實驗室中記錄的 Azure Cosmos DB 帳戶的**主要連接字串**。 這是**您在 記事本 中記錄的第三**個值。

1. 在第 6 行的 JSON 物件中，尋找 **設定。BlobContainerUrl** 属性。 請注意，目前的值是空白的：
...
1. 藉**由將 BlobContainerUrl** 和 **BlobSASToken** 屬性的值設定為**名為 images** Azure 儲存體 Blob 容器**的 URL** 屬性，以及您在此實驗室稍早記錄的 Blob **SAS 令牌**值，以更新其值。 這些是您在 記事本 中記錄的第**四**個和第**五**個值。
...
1. 從AdventureWorksCosmosContext.cs**檔案的程式**代碼編輯器索引標籤，新增下列程式代碼：
...
1. 從終端提示中將目前目錄設為 **AdventureWorks.Context**，並執行下列命令來組建 .NET Web 應用程式：
   ...
   > **注意**：如果有任何建置錯誤，請檢閱 **Allfiles > （F）：\\Allfiles\\Labs\\04\\Solution\\AdventureWorks\\AdventureWorks.Context** 資料夾中的AdventureWorksCosmosContext.cs**檔案**。
