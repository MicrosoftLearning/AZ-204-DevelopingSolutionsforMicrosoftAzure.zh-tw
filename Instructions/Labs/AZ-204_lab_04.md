...
# <a name="lab-04-construct-a-polyglot-data-solution"></a>實驗室 04：建構多語言資料解決方案
...
1.  在 [Azure Cosmos DB] **** 刀鋒視窗上，選取 [+ 建立]。

1.  在 [選取 API 選項] **** 刀鋒視窗上，選取 [Azure Cosmos DB for NoSQL] 方塊中的 [建立]。
...
1.  在 [Azure Cosmos DB 帳戶] 刀鋒視窗上尋找 [設定] **** 區段，然後選取 [金鑰] **** 連結。
...
1.  在 [Azure Cosmos DB 帳戶] 刀鋒視窗上，尋找並選取刀鋒視窗上的 [資料總管] **** 連結。
...
1.  更新 **AdventureWorksCosmosContext** 屬性的值，將其設為您稍早在此實驗中記錄的 Azure Cosmos DB 帳戶 **主要連接字串**。
...
1.  更新 **BlobContainerUrl** 屬性的值，將其設為您稍早在此實驗中記錄的 Azure 儲存體 Blob 容器 **URL** 屬性 (該容器名為 **images**)。
...
1.  從 **AdventureWorksCosmosContext.cs** 檔案的 [程式碼編輯器] 索引標籤中新增下列幾行程式碼，以從參考的 **AdventureWorks.Models** 專案中匯入 **AdventureWorks.Models** 命名空間：
