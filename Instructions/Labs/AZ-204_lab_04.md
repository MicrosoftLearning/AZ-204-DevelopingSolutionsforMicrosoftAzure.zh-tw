...
1. 在 [ ** 選取 API] 選項 ** 刀鋒視窗中，選取 ** [適用于 NoSQL ** 的 Azure Cosmos DB] 方塊中的 ** [建立 ** ]。
...
1. 按一下 ** [下一步共用存取權杖 ** ]，然後按一下 ** [產生 SAS 權杖和 URL ** ]，以使用預設設定產生存取權杖。 請記下 Blob SAS 權杖 ** 的值 ** ，因為您稍後將需要此值。
...
1. 將 BlobContainerUrl ** 和 ** BlobSASToken ** 屬性的值設定為 ** 名為 images ** Azure 儲存體 Blob 容器 ** 的 URL ** 屬性，以及您在此實驗室稍早記錄的 Blob SAS 權杖 ** 值。

1. 儲存 **appsettings.json** 檔案並將其關閉。

   > **注意**：如果您收到檔案為唯讀的提示，請選取 [覆寫]。

1. 在 ** Visual Studio Code ** 視窗中，以滑鼠右鍵按一下 ** 總管窗格中的 AdventureWorks.CoNtext ** 資料夾，然後選取 ** [在整合式終端 ** 機中開啟]。
...
1. 從 AdventureWorksCosmosCoNtext.cs ** 檔案的程式 ** 代碼編輯器索引標籤，新增下列程式碼：
