...
1. 向下捲動，直到您看到**擴增方法**為止。 按一下 [規則型]**** 選項，然後選取 [管理規則型縮放]****。
...
1. 在終端提示中執行下列命令，以確認目前的目錄設為 **Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Release\\net8.0\\net8.0\\win-x86\\publish**，也就是部署檔案的所在位置：
