...
1. 在開啟的瀏覽器視窗中，瀏覽至 `http://localhost` URL，並新增 Web API 的 **/weatherforecast** 相對路徑。
    ...
    > **注意**：完整的 URL 是 `http://localhost:[port-number]/weatherforecast`，其中 `[port-number]` 預留位置可識別 Web 應用程式透過 HTTPS 通訊協定存取的連接埠號碼。
...
1. 在 Visual **Studio Code** 視窗中的 **[檔案總** 管] 窗格中，流覽至 **bin\Debug\net8.0** 目錄。

    > **注意**：若要確認您位於 **bin\Debug\net8.0** 目錄中，請在 Visual Studio Code 的 [檔案] 功能表上，選取 [開啟資料夾]，並流覽至 **[Allfiles\\\\ Labs\\11\\入門\\Api\\bin\\Release\\net8.0\win-x86\publish**]，然後選取 [選取**資料夾**]。
