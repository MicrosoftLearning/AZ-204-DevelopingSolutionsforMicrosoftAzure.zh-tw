...
1. 在 [ **存取金鑰** ] 刀鋒視窗上，檢閱任一個 **連線 字串**\s （使用 **[顯示**] 按鈕），然後在 記事本 中記錄任**一 連線 ion 字符串** 方塊的值。 Key**** \s 是平臺管理的加密金鑰，**不會**用於此實驗室。
...
1. 檢閱產生的程式代碼，其應該包含下列內容（此程式代碼會傳回 Azure Blob 儲存體 容器中 JSON 檔案的內容，以回應 HTTP GET 要求）：
