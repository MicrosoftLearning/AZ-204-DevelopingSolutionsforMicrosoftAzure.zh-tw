...
1. 刪除 **Echo.cs** 檔案內的所有內容，然後新增下列程式碼。 其會定義名為 "Echo" 的 HTTP 觸發函式，該函式會接受 POST 要求並將要求本文當作 OkObjectResult 傳回。 ILogger 介面也用於記錄資訊。 此函式可用來測試 HTTP 要求和回應。
...
1. 刪除 **GetSettingInfo.cs** 檔案內的所有內容，然後新增下列程式碼。 此程式碼會傳回 Azure Blob 儲存體容器中 JSON 檔案的內容，以回應 HTTP GET 要求。
...
5. 在 [函數應用程式] 窗格上，選取 [函式] 區段中的 [函式] 選項。
