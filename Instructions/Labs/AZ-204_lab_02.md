...
1. 在 [ **存取金鑰** ] 刀鋒視窗上，檢閱任一 連線 **ion 字串**\s （使用 **[顯示**] 按鈕），然後在 記事本 中記錄任**一 連線 ion 字符串** 方塊的值。 Key**** \s 是平臺管理的加密金鑰，**不會**用於此實驗室。
...
1. 在 [ **建立函式應用程式** ] 刀鋒視窗上，確定 **[取用** ] 是選取的主控方案，然後按兩下 [ **選取**]。

1. 在 [**建立函數應用程式 （取用]** 刀鋒視窗的 [基本]** 索引標籤上**，執行下列動作，然後選取 **[下一步：儲存體**：
...
1. 執行下列命令，以使用 **Azure Functions Core Tools** ，在目前目錄中使用 **dotnet 隔離** 運行時間建立新的本機 Azure Functions 專案：
...
1. 刪除 **Echo.cs** 檔案內的所有內容，然後新增下列程式碼。 它會定義名為 「Echo」 的 HTTP 觸發函式，此函式會接受 POST 要求並傳回要求本文。 ILogger 介面也用於記錄資訊。 此函式可用來測試 HTTP 要求和回應。
...
1. 執行下列命令，以針對 `http://localhost:7071/api/echo` HTTP 要求本文設定為 3** 的數值**來執行 POST** REST API 呼叫測試**：

   ```cmd
   curl -X POST -i http://localhost:7071/api/echo -d 3
   ```

1. 執行下列命令，以針對 `http://localhost:7071/api/echo` HTTP 要求本文設定為 5** 的數值**來測試 **POST** REST API 呼叫：

   ```cmd
   curl -X POST -i http://localhost:7071/api/echo -d 5
   ```

1. 執行下列命令，以針對 `http://localhost:7071/api/echo` HTTP 要求本文設定為 Hello** 的**字串值來測試 **POST** REST API 呼叫：

   ```cmd
   curl -X POST -i http://localhost:7071/api/echo -d "Hello"
   ```

1. 執行下列命令來測試 **POST** REST API 呼叫`http://localhost:7071/api/echo`，HTTP 要求本文設定為 {“msg”： “Successful”}** 的 **JavaScript 物件表示法 （JSON） 值：
...
1. 在終端機內，執行下列命令，以使用 **Azure Functions Core Tools**，使用**定時器觸發**程式範本建立名為**週期**性的新函式：
...
1. 在第 17 行中，將取代 `"0 */5 * * * *"` `"0 */1 * * * *"` 為 ，將週期性頻率間隔設定為 1 分鐘，而不是 5 分鐘，並儲存變更。
...
1. 觀察大約每一分鐘發生一次的函式執行。 每個函數執行時都應將簡單的訊息轉譯至記錄。
1. 關閉目前執行的 **終端** 機應用程式。
...
1. 在終端機內，執行下列命令以使用 **Azure Functions Core Tools**，使用 **HTTP 觸發**程式範本建立名為 **GetSettingInfo** 的新函式：
...
1. 在終端機內，執行下列命令來註冊 [Microsoft.Azure.Functions.Worker.Extensions.儲存體](https://www.nuget.org/packages/Microsoft.Azure.Functions.Worker.Extensions.Storage/6.2.0) 擴充功能：

    ```powershell
    dotnet add package Microsoft.Azure.Functions.Worker.Extensions.Storage --version 6.2.0
    ```

#### 工作 4：使用 Blob 輸入撰寫 HTTP 觸發的函式程式代碼

1. 在 [開始] 畫面上選取 [Visual Studio Code] 圖格。
1. 在 [檔案]**** 功能表上，選取 [開啟資料夾]****。
1. 在開啟的 [檔案總管] 視窗中，瀏覽至 **Allfiles (F):\\Allfiles\\Labs\\02\\Starter\\func**，然後選取 [選取資料夾]。
1. 在 [Visual Studio Code] 視窗的 [Explorer] 窗格中，開啟 **GetSettingInfo.cs** 檔案。
1. 以下列程式代碼取代 GetSettingInfo** 函式自動產生的程式代碼**內容，從第 17 行開始：
...
1. 檢閱產生的程式代碼，其中包含下列內容（此程式代碼會傳回 Azure Blob 儲存體 容器中 JSON 檔案的內容，以回應 HTTP GET 要求）：
...
1. 執行下列命令，以針對 `http://localhost:7071/api/GetSettingInfo`測試 **GET** REST API 呼叫：
...
在此練習中，您已建立函式，以從記憶體帳戶傳回 JSON 檔案的內容。
...
1. 在終端機內，執行下列命令來發佈函式應用程式專案（以您稍早在此實驗室中建立的函式應用程式名稱取代 `<function-app-name>` 佔位元）：
...
5. 在 [函式應用程式] 的 [ **概觀]** 刀鋒視窗上，選取 [ **函式** ] 窗格。
