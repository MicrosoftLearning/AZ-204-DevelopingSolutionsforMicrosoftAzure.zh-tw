...
1. 在 [裝載] 索引標籤上執行下列動作，然後選取 [檢閱 + 建立]：
    ...
    ![顯示 [建立函數應用程式] 窗格上 [裝載] 索引標籤上已完成設定的螢幕擷取畫面 ](./media/l02_create_a_function_app_hosting_tab.png)
...
1. 當您收到開啟的命令提示字元時，執行下列命令，使用 **Azure Functions Core Tools 透過 **Timer trigger** 範本** 來建立名為 **Recurring** 的新函數：
...
1. 當您收到開啟的命令提示字元時，執行下列命令，使用 **Azure Functions Core Tools** 透過 **HTTP 觸發程序** 範本來建立名為 **GetSettingInfo** 的新函數：
...
#### <a name="task-3-register-azure-storage-blob-extensions"></a>工作 3：註冊 Azure 儲存體 Blob 延伸模組
...
1. #### <a name="task-4-write-http-triggered-and-blob-inputted-function-code"></a>工作 4：寫入 HTTP 觸發和 Blob 輸入函數程式碼
...
#### <a name="task-5-test-the-function-by-using-httprepl"></a>工作 5：使用 httprepl 來測試函數
...
1. 在出現的快顯對話方塊中執行下列動作： <!--Conan, as there is only sub step here, can we just combine the two as follows: In the popup dialog that appears, in the **HTTP method** list, select **GET**.-->
    - 在 [HTTP 方法] 清單中選取 [GET]。
