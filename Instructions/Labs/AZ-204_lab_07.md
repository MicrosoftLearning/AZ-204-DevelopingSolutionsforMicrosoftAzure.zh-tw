...
1. 在 [ **存取金鑰** ] 刀鋒視窗上，檢閱任一 連線 **字串**s （使用 **[顯示**] 按鈕），然後在 記事本 中記錄任**一 連線 字串** 方塊的值。 金鑰**** 是平臺管理的加密金鑰，**不會**用於此實驗室。
...
1. 在 [**函式應用程式**] 刀鋒視窗的 **[設定]** 區段中，選取 [**環境變數]** 連結。

1. 在 [ **應用程式設定]** 索引標籤中，選取 [ **+ 新增**]。 在 **[新增/編輯應用程式設定** ] 快顯對話框中輸入下列資訊：

    | 設定 | 動作 |
    |--|--|
    | [名稱] 文字輸入框 | 輸入 **儲存體連線 ionString** |
    | [值] 文字輸入框 | 使用下列語法建構值： `@Microsoft.KeyVault(SecretUri=<Secret Identifier>)`，其中 `<Secret Identifier>` 佔位元代表您稍早在此練習中記錄的秘密標識碼。 |
    | [部署位置設定] 核取方塊 | 保留預設值 |
...
1. 選取 [ **套用** ] 以關閉快顯對話框，然後返回 [ **應用程式設定** ] 區段。

1. 在 [應用程式設定]** 區段底部**，選取 [**套用**]。

    >**注意：** 您可能會收到更新應用程式設定時，應用程式可能會重新啟動的警告。 選取**確認**。 等候應用程式設定儲存完成，再繼續進行實驗。
...
1. 檢閱產生的程式代碼，其應包含下列內容（此程式代碼會傳回 儲存體連線 ionString 環境變數的值）：
...
1. 檢閱產生的程序代碼，其中包含下列內容（此程式代碼會傳回從 儲存體連線 ionString 環境變數所指定之記憶體帳戶下載的 Blob 內容）：
