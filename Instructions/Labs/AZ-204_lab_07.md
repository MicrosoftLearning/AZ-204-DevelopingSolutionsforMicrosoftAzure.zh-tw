...
1. 在 [建立存取原則] 刀鋒視窗上，選取 [1 Permissons] \(1 權限\) 區段，然後進行下列選擇：
...
1. 建立名為 **FileParser** 的新**public static**類別：
...
1. 在 **FileParser** 類別內新增下列程式碼區塊，以建立名為 **Run** 的新**public static**非同步方法。 此方法會傳回 **Task\<IActionResult\>** 類型的變數，並接收 **HttpRequest** 類型的變數，名為 request：
...
1. 再次檢閱 **FileParser.cs** 檔案內容，現在應該包含：
