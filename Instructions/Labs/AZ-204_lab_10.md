...
1.  執行下列命令，從 NuGet 匯入 7.8.1 版的 **Azure.Messaging.ServiceBus** 套件：
...
1.  新增下列程式碼，匯入 **Azure. .Queues** 套件 (從 NuGet 匯入) 中所包含的 **Azure.Messaging.ServiceBus** 命名空間：
...
1.  更新 **serviceBusConnectionString** 字串常數的值，將其設為您之前在此實驗中記錄的服務匯流排命名空間的**主要連接字串**。
...
1.  檢閱 **Program.cs** 檔案，其現在應該包含下列程式碼。 請注意，`<serviceBus-connection-string>` 預留位置代表目標 Azure 服務匯流排命名空間的連接字串：
...
1.  在 [查看模式] 索引標籤標頭，移至 [佇列] 索引標籤上，選取 [從頭查看] 按鈕。
...
1.  與先前相同，更新 **serviceBusConnectionString** 字串常數的值，將其設為您之前在此實驗中記錄的**服務匯流排**命名空間的**主要連接字串**。
...
1.  檢閱 **Program.cs** 檔案，其現在應該包含下列程式碼。 `<serviceBus-connection-string>` 預留位置代表目標 Azure 服務匯流排命名空間的連接字串：
