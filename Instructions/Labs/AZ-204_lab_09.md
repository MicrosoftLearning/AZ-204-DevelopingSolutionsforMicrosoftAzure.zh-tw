...
1. 在 [**進階**] 索引標籤的 **[事件架構] 下拉式清單中，確定**已選取 [事件方格架構****] 專案，然後選取 [**檢閱 + 建立**]。
...
1. 在 [**建立 Web 應用程式**] 刀鋒視窗的 [基本]** 索引卷標上**，執行下列動作，然後選取 **[Docker] 索引卷**標：
...
1. 新增下列程式碼：
    ...
    ```
1. 在第 7 行中，將 `<topic-endpoint>` 佔位元取代為您稍早在此實驗室中記錄的事件方格主題端點值。

1. 在第 10 行中，將 `<topic-key>` 佔位元取代為您稍早在此實驗室中記錄的事件方格主題存取金鑰值。

#### 工作 3：發佈新事件

1. **以下列程序代碼取代 Program.cs** 檔案的 **Main** 方法：
    ...
    ```
    > **注意**：若要深入瞭解 **[AzureKeyCredential](https://docs.microsoft.com/dotnet/api/azure.azurekeycredential)**
  
    > **注意**：若要深入瞭解事件方格，請按下列連結： 
